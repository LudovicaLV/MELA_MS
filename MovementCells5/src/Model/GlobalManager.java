...
import ModelCellsSim.SimulatorCellsNewForAnalyserFaster;
import ParserProg5Cells.Parser5;
	...
	public static ArrayList<Integer> createListOneD (int x){
	 ...
	 //new code - for rules
		...
		public static boolean WhileMethod(){
			if(boundary == "#Time"){
				 if (SimulatorCellsNewForAnalyserFaster.getTime() < timeTotal){return true;}
		         else{return false;}}
			else{
			if(boundary == "#Population"){
				if ((SumPopulation(nameAgentToTrack) > 0) && SimulatorCellsNewForAnalyserFaster.getTime() < 1000 ){return true;}
		    	else{return false;}}
			else{
				if(boundary == "#Pop_loc"){
					if ((EntryPopLoc(nameAgentToTrack, LocToTrack) > 0) && SimulatorCellsNewForAnalyserFaster.getTime() < 1000 ){return true;}
			    	else{return false;}}
			else{
				if ((valueForLoop < valueToCount) || SimulatorCellsNewForAnalyserFaster.getTime() < 1000){return true;}
		    	else{return false;}    
				}}}}
